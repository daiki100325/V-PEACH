...
function buildPrompt(kind: string): string {
  const common =
    ...
    "金額は円の整数（カンマ・『円』除去）。値が無い項目は空にしてください。" +
    // 財務省PDFの日付は和暦（元号）表記。西暦に変換しないと令和8年→2008年のような誤りになる。
    "【日付は必ず和暦→西暦に変換して YYYY-MM-DD で出力】PDF内の日付は和暦（元号）表記です。" +
    "『令和N年M月D日』または区切りだけの『N.M.D』形式（例: 8.6.1）で書かれています。" +
    "令和は 西暦 = 2018 + N（例: 令和8年 = 2026年。『8.6.1』→ 2026-06-01）。平成は 西暦 = 1988 + N。" +
    "西暦の2桁/4桁がそのまま書かれている場合のみそれを使い、それ以外は必ず令和として換算してください。" +
