...
| CSV-05 | シフト CSV | Step 3 で「ファイルを選択」ボタンを押してシフト CSV を選択 | ボタン横にファイル名が表示される（「選択されていません」が消える） | |
| CSV-05b | シフト CSV 未選択 | Step 3 未アップロードのまま次へ | 「選択されていません」と表示されたまま次へ進める（任意ステップ） | |
| CSV-05c | シフト CSV 取込 | HRMOS CSV をアップロード | 「✓ 取込完了：画面A・B に反映されました」と店舗別集計結果が表示 | |
...
| ID | 区分 | 手順 | 期待結果 | 結果 |
|----|------|------|----------|------|
| PLT-01 | チャート・月次 | 月次 PL 表示後 | 折れ線チャートのタイトルが「月次推移（YYYY年）」で、**選択年の1〜12月**がX軸に表示される | |
...
□ PLT-01〜04 / PLH-01〜02 チャート・Health
□ EDG-01〜03 エッジ
