...
- [ ] Cloudflare プレビュー（v3／v2）でつーくん最終確認 — **目視確認項目は §6-4 が正本**（URL 閲覧可は 2026-06-13 確認済み・細部チェック残）
- [ ] V-MINT `v3 → v2` マージ
- [ ] V-PEACH `v2 → main` マージ
...
> **対象 URL**: V-MINT `https://v3.vangvieng-app.pages.dev/`／V-PEACH `https://v2.v-peach.pages.dev/`（URL 閲覧可は 2026-06-13 確認済み）
> ⚠️ **大原則: プレビューも本番 Supabase を共有（§5-2-3）。書き込み操作（棚卸し保存・移動/発注の確定・店舗作成/休止/並べ替え・CSV取込・設定保存）はプレビューでは行わない。表示確認に徹する。**
> 数値突合の基準: 本番 URL（旧フロント）と同一 DB を見ているため、**本番アプリと同じ値が出ること＝回帰ゼロの証明**になる。
> 全項目 PASS → V-MINT `v3→v2`・V-PEACH `v2→main` マージ（§5-2-4）へ。1項目でも FAIL なら中止して報告（`multi-store` で修正→再 push→再確認）。

**V-MINT プレビュー（v3）**

- [ ] ダッシュボード: 店舗カードが 4 店舗・DB 順（事務所→馬場本店→中野店→馬場2号店）で表示され、在庫概況の数値が**本番 URL と一致**
- [ ] ダッシュボード: 「休止店舗も表示」トグルが**出ていない**（全店稼働中なので非表示が正・P5 仕様）
- [ ] 棚卸し: 店舗タブ 4 つ・任意の 1 店舗でフレーバー一覧と現在庫が表示される（**保存はしない**）
- [ ] 移動: 移動元/先セレクタに 4 店舗・事務所の特例表示（office 区別）が維持されている（**確定はしない**）
- [ ] 発注: 店舗別在庫列 4 列・店舗バッジの配色が従来どおり（office グレー／馬場本店 赤 系の既存配色維持）
- [ ] ブラウザコンソール（F12）に赤エラーが出ていない

**V-PEACH プレビュー（v2）**

- [ ] PIN ログインできる
- [ ] PL: 全店舗合計＋店舗別 PL が表示され、**直近月の営業利益・売上が本番 URL と一致**（最重要・回帰ゼロ確認）
- [ ] PL: 拠点セレクタの並び順が DB 順・「集計期間」チップが 4 店舗分表示される
- [ ] PL: 「休止店舗も表示」トグルが ON/OFF できる（**確認後は必ず OFF に戻す**＝本番アプリと連動する全社一括設定）
- [ ] PL: 3ヶ月平均・年次に切り替えてもエラーなく表示される
- [ ] データ入力: シフト CSV 取込画面が開く（**取込はしない**）
- [ ] 設定 → 店舗管理: 4 店舗（office 除く 3 shop）の一覧・`store_key` ロック表示・「＋ 新店舗を追加」ボタンが見える（**作成・編集・休止操作はしない**）
- [ ] 設定: 店舗別固定費・会社共通費・ベンチマークの既存値が表示される（**保存はしない**）
- [ ] ブラウザコンソール（F12）に赤エラーが出ていない

**確認後の後始末**

- [ ] `app_ui_settings.show_inactive_stores` が `false` のままであること（トグルを触った場合は OFF へ戻したか）
