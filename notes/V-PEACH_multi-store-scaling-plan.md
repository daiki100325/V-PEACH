...
> 進捗: **P1・P2 ✅ 完了（2026-06-11）**／**P0 ✅ 完了（2026-06-13）**（保険ブランチ V-MINT `v3`／V-PEACH `v2` を subtree push で新規生成・両プレビュー稼働確認・本番ブランチ無傷）／**P3 ✅ 完了（2026-06-12）**（2026-06-11 全実装タスク完了〔§5-4 体制で Sonnet×2＋Opus×1 サブエージェント並列実装 → Fable レビュー・ビルド検証〕→ 2026-06-12 つーくんの画面スモーク全項目 PASS: V-MINT ダッシュボード/棚卸し/移動/発注/office特例・V-PEACH PL/シフトCSV取込〔馬場2号店 遅番土日祝補正含む〕）／**P4 ✅ 完了（2026-06-13）**（前半: `create_store_atomic` RPC・店舗管理セクション／後半: 追加ウィザード／2026-06-13 統合 e2e PASS＝GUI 追加→両アプリ反映・`requirements`/`how-to-use` ドキュメント同期済み）／**P5 ✅ 完了（2026-06-13）**（休止表示トグル両アプリ・`closed_at` 翌月以降の集計除外〔按分分母含む〕・csvImporter 動的化／2026-06-13 統合 e2e PASS＝休止化→トグル連動→決算閲覧→翌月除外。e2e で発見した PL 集計期間チップの出し分け漏れを修正済み）／**P6 ✅ 完了（2026-06-13）**（localhost 目視確認〔§6-4〕全項目 PASS→本番 push〔V-MINT→`v2` `e0a00c9..905df89`・V-PEACH→`main` `cd5733f..7d8ac49`、両 fast-forward〕→ Cloudflare 本番ビルド → **本番スモーク PASS**。**マルチストア改修 go-live 完了**）／**P7 ⬜ 未着手（go-live 後のレガシー除去・§6-2）**。**次回再開時は P7 着手判断から**（旧ピボット RPC `DROP`・`*_v2` リネーム等。破壊的のため §5-3-4 ルール適用）。最新は §6 進捗帳票を参照。
...
| **P6** | ✅ | 2026-06-13 | **本番反映（go-live）完了**。`multi-store` HEAD から本番へ直接 subtree push（V-MINT→`v2` `e0a00c9..905df89`／V-PEACH→`main` `cd5733f..7d8ac49`、両 fast-forward・diverge なし）→ Cloudflare 本番ビルド → **本番スモーク PASS**（V-PEACH PL 直近月売上/営業利益・V-MINT 在庫1モード・コンソール赤エラーなし） | 本番 URL に反映・スモーク確認完了 ✅ |
...
**P6 — 本番反映（go-live）** — ✅ 2026-06-13 完了（本番 push → Cloudflare ビルド → 本番スモーク PASS）
- [x] `test-plan` 全件 done を確認 ✅ 2026-06-13（実体は P3 画面スモーク全項目 PASS〔2026-06-12〕＋ §6-3 P4×P5 統合 e2e 全手順 PASS〔2026-06-13〕）
- [x] localhost dev サーバーでつーくん最終確認 ✅ 2026-06-13 — §6-4 全項目 PASS（V-MINT: 日付フォーマット・Vue warn バグ発見→即修正済み／V-PEACH: 初回全項目 PASS。集計期間チップ=合計非表示は仕様どおり確認）
- [x] V-MINT → 本番 `v2` 反映 ✅ 2026-06-13 — `multi-store` HEAD（コミット `3361cfc`）から `git subtree push --prefix=V-MINT2.0 V-MINT v2`。fast-forward（`e0a00c9..905df89`）・diverge なし。※保険 `v3` 経由でなく直接 push（今回バグ修正が `v3` 未反映のため。プラン §5-2-4 が許容する方式）
...
| # | 手順 | 期待結果 |
|---|------|---------|
| 1 | V-PEACH 設定→店舗管理→「＋ 新店舗を追加」。テスト値: 表示名「テスト店」／key `test_store`／適用開始月=当月／固定費5項目=適当な値（例: 家賃100000）／シフト既定プリセットのまま確定 | ウィザード成功・一覧に「テスト店」が出現 |
| 2 | DB 確認（MCP 読み取り）: `stores`・`pe_store_settings`・`pe_store_settings_revisions`・`pe_store_shift_rules` を `test_store` で SELECT | 1行＋1行＋1行＋6行が揃う（`create_store_atomic` のアトミック性） |
| 3 | 両アプリ反映確認: V-PEACH PL セレクタ／V-MINT 棚卸し・移動・発注・ダッシュボード | 全箇所に「テスト店」が出現（リロードのみ・コード変更なし）＝**P4 e2e ✅ 条件** |
| 4 | V-PEACH 店舗管理で「テスト店」を休止（確認ダイアログ→`closed_at`=当日） | 両アプリの通常モードから消える。V-MINT ダッシュボードに「休止店舗も表示」トグルが出現 |
| 5 | V-PEACH PL でトグル ON → V-MINT ダッシュボードをリロード | V-MINT 側もトグル ON になっている（`app_ui_settings` DB 共有・全社一括） |
| 6 | トグル ON 状態で PL・ダッシュボード | 「テスト店（休止）」がグレー表示で出る・過去 PL 閲覧可＝**P5 e2e ✅ 条件** |
| 7 | 閉店翌月の期間で PL 表示 | テスト店が集計・按分分母に混ざらない（`isStoreOpenForPeriod` 除外） |
| 8 | 後片付け: テストデータ削除は **破壊的 SQL**（§5-3-4 ルール: 対象行数 SELECT→FK 影響→ロールバック手順→承認→実行）で `pe_store_shift_rules`→`revisions`→`pe_store_settings`→`stores` の順に DELETE。または休止のまま残置でも可（運用上は無害） | 残骸ゼロ（または休止残置を DECISIONS に記録） |
| 9 | 完了処理: §6-1 の P4「GUI 追加→…e2e」・P5「休止店舗の決算閲覧 e2e」を ✅ 化、§6 表の P4・P5 を ✅ に。P4 残のドキュメント同期（`requirements`/`how-to-use`）も実施 | P4・P5 完了 → 次は **P6 go-live（§5-2-4）** |

### 6-4. P6 本番マージ直前のプレビュー目視確認チェックリスト（つーくん実施）

> **✅ 2026-06-13 全項目 PASS（localhost dev サーバーで実施）。** V-MINT で日付フォーマット mm/dd/yyyy バグ・`requestCurrentStoreName` Vue warn を発見→即修正 PASS。V-PEACH は初回全項目 PASS。**次は本番マージ（§5-2-4）のみ。**
> ~~対象 URL: V-MINT `https://v3.vangvieng-app.pages.dev/` / V-PEACH `https://v2.v-peach.pages.dev/`~~

**V-MINT（localhost:5173）**

- [x] ダッシュボード: 店舗カードが 4 店舗・DB 順（事務所→馬場本店→中野店→馬場2号店）で表示される ✅ 2026-06-13
- [x] ダッシュボード: 「休止店舗も表示」トグルが**出ていない** ✅ 2026-06-13
- [x] 棚卸し: 店舗タブ 4 つ・フレーバー一覧と現在庫が表示される ✅ 2026-06-13（実施日 mm/dd/yyyy バグ→修正済み）
- [x] 移動: 移動元/先セレクタに 4 店舗・office 特例表示が維持されている ✅ 2026-06-13（実施日 mm/dd/yyyy バグ→修正済み）
- [x] 発注: 店舗別在庫列 4 列・店舗バッジの配色が従来どおり ✅ 2026-06-13
- [x] ブラウザコンソール: `requestCurrentStoreName` Vue warn → 修正済み・赤エラーなし ✅ 2026-06-13

**V-PEACH（localhost:5174）**

- [x] PIN ログインできる ✅ 2026-06-13
- [x] PL: 全店舗合計＋店舗別 PL が表示される ✅ 2026-06-13
- [x] PL: 拠点セレクタの並び順が DB 順・「集計期間」チップが店舗別表示で出る（**合計選択時は非表示＝仕様どおり**） ✅ 2026-06-13
- [x] PL: 「休止店舗も表示」トグルが ON/OFF できる・確認後 OFF 復帰済み ✅ 2026-06-13
- [x] PL: 3ヶ月平均・年次に切り替えてもエラーなし ✅ 2026-06-13
- [x] データ入力: シフト CSV 取込画面が開く ✅ 2026-06-13
- [x] 設定 → 店舗管理: 3 shop 一覧・`store_key` ロック表示・「＋ 新店舗を追加」ボタンが見える ✅ 2026-06-13
- [x] 設定: 店舗別固定費・会社共通費・ベンチマークの既存値が表示される ✅ 2026-06-13
- [x] ブラウザコンソール: 赤エラーなし ✅ 2026-06-13

**確認後の後始末**

- [x] `app_ui_settings.show_inactive_stores` が `false` のままであること ✅ 2026-06-13
