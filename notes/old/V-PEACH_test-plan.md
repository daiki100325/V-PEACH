...
### 5.2 月次入力モード（Step 0〜6）

> ステップ構成: Step 0（対象月選択）→ **Step 1（売上CSV）→ Step 2（売上プレビュー）→ Step 3（シフトCSV）→ Step 4（人件費プレビュー）→ Step 5（給与総額）→ Step 6（最終確認）**。1/6〜6/6 のバッジは全ステップに表示。新規月は CSV 必須・既存月再編集モードでは CSV 未アップロード時に DB 既存値を使用可。手入力モードは廃止済み。
>
...
| CSV-02 | Step 1（1/6）売上CSV一括 | 「CSVファイルをまとめて選択」ボタンから 3店舗 × Airメイト+Airレジ の6ファイルを一度に選択（各ファイル名に「馬場本店」「中野店」「馬場2号店」を含む） | 店舗・種別を自動判定してスロットが埋まり「次へ」が活性化する | ✅ |
| CSV-02b | 削除ボタン | 誤アップロードしたスロットの「削除」ボタンを押す | 当該スロットのみクリアされ、もう片方は残る | ✅ |
| CSV-02c | 種別不明ファイル | ヘッダーが Airメイト/Airレジ のどちらにも一致しない CSV をアップロード（ファイル名には店舗名あり） | 該当店舗カードに黄バッジ警告が表示され「次へ」が活性化しない | ✅ |
| CSV-02c2 | 店舗名未検出 | ファイル名に「馬場本店」「中野店」「馬場2号店」のいずれも含まれない CSV をアップロード | 上部グローバル警告エリアに「ファイル名に店舗名が含まれていません」が表示される | ✅ |
| CSV-02d | 再編集モード（売上） | 全店の月次レコードが揃った月を選び、CSV を一切アップロードせず「次へ」 | 各店舗カードに「DB既存値を使用中」バナー＋既存売上値が表示され、「次へ」が活性化する | ✅ |
| CSV-02e | 再編集モード（部分上書き） | 上記の状態で 1 店舗のみ CSV をアップロード（ファイル名に店舗名含む） | その店舗は CSV 値、他店舗は DB 既存値のまま次へ進める | ✅ |
| CSV-03 | ヘッダー自動判定 | ファイル名に「中野店」を含み・ヘッダーが Airメイト形式の CSV をアップロード | 中野店の「Airメイト」スロットに自動振り分けされる | ✅ |
| CSV-04 | Step 2（2/6）売上プレビュー | Step 1 後「次へ」 | 店舗別に提供売上（割引後）・物販売上・割引総額・前月キャッシュ参照日数が表示される。ソース（CSV / DB既存値）バナー表示。手入力欄なし | ✅ |
| CSV-05 | Step 3（3/6）シフトCSV選択 | 「ファイルを選択」ボタンを押して `vangvieng_shifts_YYYYMM.csv` を選択 | ボタン横にファイル名が表示される（「選択されていません」が消える） | ✅ |
| CSV-05b | シフトCSV必須（新規月） | 新規月で Step 3 未アップロードのまま | 「未アップロード（次へ進むには CSV が必要です）」赤表示・「次へ」が無効 | ✅ |
| CSV-05b2 | シフトCSV省略（再編集モード） | 既存月で Step 3 未アップロード | 「DB既存値を使用中。上書きしたい場合のみ CSV をアップロード」バナー表示・「次へ」が活性化 | ✅ |
| CSV-05c | シフトCSV取込 | HRMOS シフト CSV をアップロード | 「✓ 取込完了：人件費プレビューに反映されました」と店舗別集計（6h / 7.5h × バイト/りょーさん・営業日数）が表示。重みつき合計時間も併記 | ✅ |
| CSV-05d | シフトCSV警告 | マスタ未登録のスタッフ/勤務区分を含む CSV をアップロード | 黄バナー「⚠ 警告」に未登録要素が列挙される（取込自体は実施） | ✅ |
| CSV-05e | シフトCSVマスタ未登録エラー | `pe_hrmos_staffs` / `pe_hrmos_segments` が空の状態で取込 | 赤エラー「HRMOS スタッフ／勤務区分マスタが未登録です」が表示され、人件費プレビュー未反映 | ✅ |
| CSV-06 | Step 4（4/6）人件費プレビュー | Step 3 後「次へ」 | バイト枠・りょーさん枠の 6h / 7.5h 枠数と重みつき枠数が読み取り専用で表示。ソース（シフトCSV / DB既存値）バナー表示。入力欄なし | ✅ |
| CSV-08 | Step 5（5/6）給与総額入力 | 全店バイト給与＋交通費総額を入力 | 数値必須（0円可）、未入力時は「次へ」が無効。保存後 `pe_monthly_company_records` に upsert される | ✅ |
| CSV-09 | Step 6（6/6）最終確認 | 「次へ」 | 店舗別に売上・枠数・全店給与＋交通費合計が一覧表示 | ✅ |
| CSV-10 | 保存（CSV経路） | 確認 → 保存 | `pe_monthly_records` に 3 行 upsert（売上＋枠数）。`pe_monthly_company_records` に当月行 upsert。`pe_daily_sales_cache` に CSV あり店舗分のみ upsert、CSV を新規取込した店舗のみ当月度 start_date より古いキャッシュ削除 | ✅ |
...
| CSV-12 | CSVパースエラー | 不正な CSV（ヘッダー欠落等）をアップロード | 該当店舗スロットにエラー表示、先に進めない | ✅ |
...
| PLT-02 | チャート・3ヶ月平均 | 3ヶ月平均 PL 表示後 | チャートタイトルが「月次推移（YYYY年）」で、選択年1〜12月がX軸。PL数値は選択月含む直近3ヶ月平均 | ⏭ チャートは月次生値を表示（仕様扱い：全12ヶ月分の前月プリフェッチが必要なため実装コスト大） |
| PLT-03 | チャート・年次 | 年次 PL 表示後 | チャートタイトルが「年次推移（直近12年）」で、X軸が「2026年」「2027年」…と年単位で表示される | ✅ |
| PLT-04 | チャート・年跨ぎ | 月次で翌年1月を選択 | 翌年の1〜12月がX軸に切り替わる | ✅ |
| PLT-05 | チャート・カテゴリトグル | 売上/原価/販管費/利益/FLR比 のカテゴリーから個別指標をON/OFF | 二重Y軸（左:金額、右:%）でラインが追加・削除される。初期表示は「税込総売上・F比・L比・R比・会社手残り」 | ✅（修正済み: ①展開不可→`style` の `.has()`→`.includes()` ②トグル無反映→Chart.js を `markRaw()` 化しリアクティブ Proxy による `update()` 不発を解消） |
...
| EDG-02 | ネットワーク | Supabase 切断状態で PL 表示 | エラーアラート、クラッシュしない | ⏭ 環境都合 |
...
> 進捗（2026-06-01 更新）: **全テスト完了**。§5 の機能テスト一巡 + PLT-05（トグル不具合）修正クリア + finance-spec サンプル検証 + 本番スモークまで done。スキップ項目（SET-15 / PLT-02 / EDG-02）は既知・環境都合・仕様扱い。
...
☑ SET-10〜15 HRMOS マスタ管理・祝日マスタ（CSV 取込含む）※SET-15 は alert 非表示で ⏭（既知・無視）
☑ INP-00〜13 / CSV-02〜12 月次入力（Step 0 対象月選択 → 売上CSV → 売上プレビュー → シフトCSV → 人件費プレビュー → 給与総額 → 最終確認 + 再編集モード + 人件費按分確認）
☑ PLM-00〜08 月次 PL（§4.1 レガシー / §4.2 新方式 / FLR比 / 集計期間バッジ）
☑ PLR-01〜02 / PLA-01〜02 集計モード
☑ PLT-01〜05 / PLH-01〜02 チャート・Health ※PLT-02 は仕様扱いで ⏭、PLT-05 は修正後クリア
☑ EDG-01〜04 エッジ ※EDG-02 は環境都合で ⏭
...
| 基準 | 条件 |
|------|------|
| **必須** | §5 の COM / SET（SET-10〜15 含む）/ 月次入力（INP・CSV）/ PLM をすべて ✅（EDG-02 は環境都合で ⏭ 可） |
