...
**CSV インポートフロー（6 ステップ）**
- Step 1：売上 CSV アップロード。店舗ごと 1 ボックスへ複数ファイル選択（Airメイト CSV + Airレジ CSV を同時選択）× 3店舗 = 3ボックス。ヘッダー内容（「カテゴリー」「売上合計」vs「集計期間」「割引額」）から種別を自動判定して振り分け。再選択は追加マージ方式（既存スロット温存・後勝ちで上書き）。誤アップロードは「削除」ボタンでスロット単体クリア。
...
- Step 3：HRMOS シフト CSV 取込。`vangvieng_shifts_YYYYMM.csv` をアップロードすると各店舗の枠数（バイト・りょーさん）を自動算出。新規月は必須・再編集モードは DB 既存値で代用可。
