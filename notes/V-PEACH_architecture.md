...
- **リトライ＋控えめフォールバック**（2026-06-15・[[V-PEACH/DECISIONS]] ADR-20260615-02）: 429（レート制限）/503（高負荷）/5xx は **同一モデルで最大2回 retry+backoff**（応答の `retryDelay`/`Please retry in Xs` をパース・既定5s・上限30s）。なお失敗したら `GEMINI_MODEL_FALLBACKS`（カンマ区切り・**既定 OFF**）の順にフォールバック。精度最優先のため主軸は `gemini-2.5-flash` 固定で、フォールバックは「PDF入力＋responseSchema 対応」モデルのみ許容（gemma 等テキスト専用は不可）。`thinkingConfig` は 2.5-flash 系のみ付与（pro/2.0 で 400 を避ける）。全滅時はレート制限を示す日本語ヒント付き 502 を返す。応答 `model` フィールドに実際に成功したモデル名を返す。
