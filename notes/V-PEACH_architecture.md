...
- **モデル選定＋リトライ＋フォールバック**（2026-06-15・[[V-PEACH/DECISIONS]] ADR-20260615-02）: 無料枠 RPD が決め手で**主軸は `gemini-3.1-flash-lite`（RPD 500・実測 7s）**。旧主軸 `gemini-2.5-flash` は RPD 20 で枯渇（`free_tier_input_token_count` 超過）。429/503/5xx は **同一モデルで最大3回 retry+backoff**（`retryDelay` パース・上限60s）→ なお失敗（4xx 含む）なら `GEMINI_MODEL_FALLBACKS`（既定 `gemini-3.5-flash,gemini-2.5-flash`）へ。**全体待機予算 `GLOBAL_BUDGET_MS=90s`** で打ち切り（Edge ~150s 制限の安全ガード）。`thinkingConfig` は flash 系のみ付与。`gemma`（PDF/構造化不可）・`gemini-3.1-pro`（無料枠0）は不採用。詳細は [[V-PEACH/TROUBLESHOOTING]]。精度最優先のため主軸は `gemini-2.5-flash` 固定で、フォールバックは「PDF入力＋responseSchema 対応」モデルのみ許容（gemma 等テキスト専用は不可）。`thinkingConfig` は 2.5-flash 系のみ付与（pro/2.0 で 400 を避ける）。全滅時はレート制限を示す日本語ヒント付き 502 を返す。応答 `model` フィールドに実際に成功したモデル名を返す。
